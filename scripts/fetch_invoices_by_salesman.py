#!/usr/bin/env python3
"""
Laudus Invoices By Salesman Fetcher
For manual data loading with specific year and month
Endpoint: /reports/sales/invoices/bySalesman
"""
...
ENDPOINT = {
    'name': 'InvoicesBySalesman',
    'path': '/reports/sales/invoices/bySalesman',
    'collection': 'invoices_by_salesman'
}
...
class LaudusAPIClient:
    ...
    def fetch_invoices_by_salesman(self, date_from: str, date_to: str) -> Optional[List[Dict]]:
        """Fetch invoices by salesman data"""
        try:
            params = {
                'dateFrom': date_from,
                'dateTo': date_to
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("  CARGA FACTURAS POR VENDEDOR")
    logger.info("=" * 60)
    ...
    while attempt < max_attempts and not completed:
        ...
        data = api_client.fetch_invoices_by_salesman(first_day, last_day)
